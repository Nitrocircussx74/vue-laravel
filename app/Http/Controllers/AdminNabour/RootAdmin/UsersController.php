...
use App\Http\Controllers\AdminNabour\Officer\AccountController;
use Request;
...
class UsersController extends Controller {
	...
	public function newUsers() {
		...
		if(Request::ajax()) {
			...
			return view('Backend.users.new-user-list')->with(compact('users','provinces'));
		} else {
			...
			return view('Backend.users.new-list')->with(compact('users','property_list','provinces'));
		}
	}
	...
	public function allUsers() {
		...
		if(Request::ajax()) {
			...
			return view('Backend.users.all-users-list-element')->with(compact('users','provinces'));
		} else {
			$users = User::with('property')->whereHas('property', function($query){
                $query->where('is_demo', false);
			...
			return view('Backend.users.all-users-list')->with(compact('users','property_list','provinces'));
		}
	}
	...
	public function export(Request $request) {
		...
		if($users->count()) {
			return view('Backend.users.export-list')->with(compact('users','provinces','request'));
		} else {
	...
	public function internalUserEmail ($id) {
		if (Request::isMethod('get')) {
			...
			if($user) {
				...
				return view('Backend.users.email-internal-user')->with(compact('user'));
			}
		} else {
	...
	public function externalUserEmail () {
		if (Request::isMethod('get')) {
			@session_start();
			$_SESSION['allow_upload_kc'] = false;
			return view('Backend.users.email-external-user')->with(compact('user'));
		} else {
